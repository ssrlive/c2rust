...
fn test(file_name: &str) {
    ...
    cmd.arg("-A").arg("warnings")
        .arg("--crate-name").arg(path.file_stem().unwrap())
        .arg("--crate-type").arg("rlib")
        .arg("--error-format").arg("json")
        .arg(&fixed_path)
        .stderr(File::create(&json_path).unwrap());
    let status = cmd.status().unwrap();
    assert_eq!(status.code(), Some(1), "command {cmd:?} exited with code {status:?}");
    ...
    cmd.arg(&script_path)
        .arg(&json_path);
    let status = cmd.status().unwrap();
    assert!(status.success(), "command {cmd:?} exited with code {status:?}");
