...
fn run(tcx: TyCtxt) {
    ...
    for &ldid in &all_fn_ldids {
        let info = func_info.get_mut(&ldid).unwrap();
        let mut local_equiv = LocalEquivSet::new(
            info.acx_data.local_ptr_base(),
            info.acx_data.num_pointers(),
        );
        ...
        match r {
            ...
            Err(pd) => {
                acx.gacx.mark_fn_failed(ldid.to_def_id(), DontRewriteFnReason::DATAFLOW_INVALID, pd);
            }
        };
    ...
    for &ldid in &all_fn_ldids {
        ...
        let info = func_info.get_mut(&ldid).unwrap();
        let (local_base, local_count, local_equiv_map) =
            info.local_equiv.renumber(&global_equiv_map, &mut local_counter);
        ...
        for ldid in tcx.hir().body_owners() {
            ...
            for (local, decl) in mir.local_decls.iter_enumerated() {
                ...
                for ptr in ptrs {
                    ...
                    let parent = if acx.ptr_is_global(ptr) { None } else { Some(ldid) };
...
fn assign_pointer_ids<'tcx>(
    ...
    for &ldid in all_fn_ldids {
        ...
        eprintln!("local pointerid range: {} .. {}", acx.local_ptr_base(), next_base);
...
fn pdg_update_permissions_with_callback<'tcx>(
    ...
    for g in &graphs.graphs {
        for n in &g.nodes {
            ...
            callback(asn, ldid, ptr, ptr_is_global, g, node_info);
